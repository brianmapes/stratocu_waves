...
for i = 1:numCases
    ...
    % bar(log10(xCenters), data(:, i), barWidth, 'FaceColor', colors(i,:), 'EdgeColor','k'); % Use specified color
    % bar( xCenters, data(:, i), barWidth, 'FaceColor', colors(i,:), 'EdgeColor','k'); % Use specified color
    stem( xCenters, data(:, i), 'color', colors(i,:)); % Use specified color
end
...
ylabel('abs(spec) x 2/S');             % Or your y-axis label
...
xlim( ([4 300]) ); 
set(gca,'XScale','log'); 
% set(gca,'Xtick',([5 10 20 50 100 200 300]));

% instead do this,https://stackoverflow.com/questions/22410715/
...
% Add a legend
legend({'256','128', '64', '32', '16', '8'}, 'Location', 'best'); % Customize labels
